	...
	*/
	_fetchWeatherStatus(postalCode) {
		return new Promise((response, reject) => {
			let wunderground = new Wunderground(CONST.WUNDERGROUND_KEY)

			var parts = postalCode.match(/^([A-Z]{1,2}\d{1,2}[A-Z]?)\s*(\d[A-Z]{2})$/);
  		parts.shift();
  		let formattedPostalCode = parts.join(' ')
	...
	_createDatabaseRecord(executionTime, nestData, weatherData) {
		return {
			...
			devices : Object.keys(nestData.device).reduce((acc, id) => {
				let tempScale = nestData.device[id].temperature_scale.toLowerCase()
				let postalCode = nestData.device[id].postal_code
				let weather = ((weatherData[postalCode] || {}).current_observation || {})
				acc[id] = {
					id : id,
	...
	recordNow() {
		...
		return db.cleanup().then(() => {
			return this._fetchNestStatus().then(nestData => {
				var postalCodes = Object.keys(nestData.device).map((id) => { return nestData.device[id].postal_code })
				postalCodes = Array.from(new Set(postalCodes))
				return this._fetchMultipleWeatherStatus(postalCodes).then(weatherData => {
					let now = new Date()
					let record = this._createDatabaseRecord(now.getTime() - startTime.getTime(), nestData, weatherData)
